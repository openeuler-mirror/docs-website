...
def run_check(all_files):
    ...
    urls = set(all_urls) - set(precise_whites)
    ...
    for pattern in re_white_urls:
        matched = {url: 200 for url in urls if is_white_url(pattern, url)}
        white_url_save.update(matched)
        urls -= matched.keys()

    # 多线程检查剩余URL
    pool = []
    for url in urls:
        t = threading.Thread(target=update_url_status_to_json, args=(url,))
        t.start()
        pool.append(t)
...
def generator_report(all_files):
    ...
    for file_name in all_files:
        urls = set(get_file_urls(file_name))

        # 先过滤白名单URL
        white_matched = [url for url in urls
                         if url in white_urls or
                         any(is_white_url(pattern, url) for pattern in white_urls if "*" in pattern)]
        urls -= set(white_matched)

        for url in urls:
            if url_status.get(url, 200) == 404:
                msg_list.extend(location_error_line(file_name, url))
