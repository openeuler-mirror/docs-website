...
const props = defineProps({
  ...
  // 是否递归渲染子菜单，默认 false，最多只渲染第一层的children
  recursion: {
    type: Boolean,
    default: false,
  },
...
<template>
  <aside class="doc-menu">
    <OScroller id="menuScrollDom" show-type="hover" size="small" disabled-x>
      ...
      <OMenu v-else class="chapter-menu">
        <div
          v-for="(chapter, i) in items"
          :key="chapter.id"
          class="chapter-item"
          :class="[chapter.isManual || chapter.children?.length === 0 ? 'chapter-item-is-manual' : '']"
        >
          <!-- 第二层-手册节点 -->
