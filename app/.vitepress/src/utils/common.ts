 ...
 * @returns 取消禁用函数
 */
export function disableTriggerScrollEvent(dom: Element) {
  let tempScrollEvent: EventListenerOrEventListenerObject | null = (e: Event) => {
    e.stopImmediatePropagation();
  };

  dom.addEventListener('scroll', tempScrollEvent, true);

  return () => {
    dom.removeEventListener('scroll', tempScrollEvent!, true);
 ...
 * @param {boolean} disableScrollEvent 禁止触发滚动事件
 */
export const scrollToTop = (top: number = 0, smooth: boolean = true, disableScrollEvent = false) => {
  if (!isClient) {
    return;
  }
  ...
  let enableScrollEvent: null | (() => void);
  if (disableScrollEvent) {
    enableScrollEvent = disableTriggerScrollEvent(dom);
  }
