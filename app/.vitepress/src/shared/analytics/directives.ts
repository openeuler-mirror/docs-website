  ...
  mounted(el, binding) {
    ...
    if (isBubble && !binding.modifiers.noTrigger) {
      ...
      el.addEventListener(originalEvent, (ev) => {
        if (handledEventSet.has(ev)) { // 判断该标准事件是否已被后代元素处理过
          return;
        }
    ...
    el.addEventListener(listeningEvent, (ev: Event) => {
      ...
      if (isBubble) {
        ...
        if (isFromBubble(ev)) {
          if (isObj(bindingValue)) {
            Object.assign(ev.detail.data, bindingValue);
          }
          if (typeof bindingValue === 'function') {
            const data = bindingValue(ev, ev.detail.data);
            if (isObj(data)) {
              Object.assign(ev.detail.data, data);
            }
          }
        }
      } else if (binding.modifiers.catchBubble) {
        if (isFromBubble(ev)) {
          let currentData: AnalyzeData;
          if (typeof bindingValue === 'function') {
            currentData = bindingValue(ev, ev.detail.data);
          } else if (isObj(bindingValue)) {
            // 合并从子元素冒泡上来的数据，并调用上报
            currentData = {
              ...bindingValue,
              properties: {
                ...(bindingValue as AnalyzeDataInternal).properties,
                ...ev.detail.data,
              },
            };
          }
